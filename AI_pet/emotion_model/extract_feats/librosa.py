...
def features(file_name, mfcc=True, chroma=True, mel=True):
    with soundfile.SoundFile(file_name) as sound_file:
        ...
        result = np.array([])
        if mfcc:
            sample_mfcc = np.mean(librosa.feature.mfcc(y=x[:,0], sr=sample_rate, n_mfcc=40).T, axis=0)
        ...
        if mel:
            sample_mel = np.mean(librosa.feature.melspectrogram(x[:, 0], sr=sample_rate).T, axis=0)
...
def get_data(config, data_path, feature_path, train : bool):
    if train==True:
        ...
        # for file in glob.glob(data_path):
        feature = features(data_path)
        data = [[data_path, feature, -1]]
